...
struct object_type tuplehash_type = {TUPLE_OP};
...
static struct bucket *bucket_at(struct tuplehash_st *map, size_t index) {
...
static uint64_t get_hash(struct tuplehash_st *map, const void *key) {
    return map->hash(key, map->seed0, map->seed1) << 16 >> 16;
}

// tuplehash_new_with_allocator returns a new hash map using a custom allocator.
// See tuplehash_new for more information information
                            ...
                            void *udata)
{
    ...
    // tuplehash_st + spare + edata
    size_t size = sizeof(struct tuplehash_st) + bucketsz * 2;
    struct tuplehash_st *map = malloc(size);
    if (!map) {
        return NULL;
    }
    memset(map, 0, sizeof(struct tuplehash_st));
    map->elsize = elsize;
    ...
    map->elfree = elfree;
    map->udata = udata;
    map->spare = ((char*)map)+sizeof(struct tuplehash_st);
...
// tuplehash_new returns a new hash map.
// Param `elsize` is the size of each element in the tree. Every element that
// is inserted, deleted, or retrieved will be this size.
// Param `cap` is the default lower capacity of the tuplehash_st. Setting this to
...
// comes with two helper functions `tuplehash_sip()` and `tuplehash_murmur()`.
// Param `compare` is a function that compares items in the tree. See the 
// qsort stdlib function for an example of how this function works.
// The tuplehash_st must be freed with tuplehash_free().
...
struct tuplehash_st *tuplehash_new(size_t elsize, size_t cap,
                                   ...
                                   void *udata)
{
    return tuplehash_new_with_allocator(
...
static void free_elements(struct tuplehash_st *map) {
    if (map->elfree) {
...
// tuplehash_clear quickly clears the map.
// Every item is called with the element-freeing function given in tuplehash_new,
// if present, to free any data referenced in the elements of the tuplehash_st.
...
static bool resize(struct tuplehash_st *map, size_t new_cap) {
    struct tuplehash_st *map2 = tuplehash_new_with_allocator(map->malloc, map->realloc, map->free,
...
// tuplehash_set inserts or replaces an item in the hash map. If an item is
// replaced then it is returned otherwise NULL is returned. This operation
// may allocate memory. If the system is unable to allocate additional
// memory then NULL is returned and tuplehash_oom() returns true.
...
// tuplehash_get returns the item based on the provided key. If the item is not
...
void *tuplehash_get(struct tuplehash_st *map, const void *item) {
	...
	for (;;) {
		...
		if (bucket->hash == hash &&
            map->compare(item, bucket_item(bucket), map->udata) == 0)
        {
...
// tuplehash_probe returns the item in the bucket at position or NULL if an item
// is not set for that bucket. The position is 'moduloed' by the number of 
// buckets in the tuplehash_st.
...
// tuplehash_delete removes an item from the hash map and returns it. If the
...
// tuplehash_count returns the number of items in the hash map.
size_t tuplehash_count(struct tuplehash_st *map) {
    return map->count;
}

// tuplehash_free frees the hash map
// Every item is called with the element-freeing function given in tuplehash_new,
// if present, to free any data referenced in the elements of the tuplehash_st.
...
// tuplehash_oom returns true if the last tuplehash_set() call failed due to the
// system being out of memory.
bool tuplehash_oom(struct tuplehash_st *map) {
    return map->oom;
}

// tuplehash_scan iterates over all items in the hash map
...
// tuplehash_iter iterates one key at a time yielding a reference to an
// entry at each iteration. Useful to write simple loops and avoid writing
// dedicated callbacks and udata structures, as in tuplehash_scan.
//
...
// modified.
//
// Note that if tuplehash_delete() is called on the tuplehash_st being iterated,
...
// tuplehash_sip returns a hash value for `data` using SipHash-2-4.
...
// tuplehash_murmur returns a hash value for `data` using Murmur3_86_128.
...
int tuplehash_is_null(const struct tuplehash_st *res) {
    return (res == NULL || res->size == 0);
}

// Class methods
void tuplehash_resize(struct tuplehash_st *res, size_t size) {
